...
def main():
    ...
    ok, reason = check_normal_request_works(
        device
    )

    checks.append(
        (
            "normal request succeeds regardless "
            "of device",
            ok,
            reason
        )
    ...
    ok, reason = check_gpu_only_request(
        device
    )

    label = (
        "GPU-only request fails cleanly on CPU "
        "(400, clear message)"
        if device == "cpu"
        else
